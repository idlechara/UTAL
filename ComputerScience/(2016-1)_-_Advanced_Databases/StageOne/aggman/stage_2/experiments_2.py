...
def log(string):
    print string
    with open(logging_route, "a") as f:
        f.write(string)
        f.write("\n")
...
def followed_execution(params, btree = False, hash = False, test = False):
...
def db_assignment(test):
    ...
    konnect()
    followed_execution(databases[0], btree=False, hash=False, test=test)
    #drop(non_indexed=True)
    followed_execution(databases[1], btree=True, hash=False, test=test)
    # drop(btree=True)
    followed_execution(databases[2], btree=False, hash=True, test=test)
    # drop(hash=True)
    followed_execution(databases[3], btree=True, hash=True, test=test)
    # drop(full=True)
