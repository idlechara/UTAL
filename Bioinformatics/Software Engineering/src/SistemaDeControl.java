...
public class SistemaDeControl {
	...
	/**
	 * Valida las zonas e inicia la reparacion
	 */
	public void validarZonas(){
		int caso = this.zonas.validarZonas();
		if(caso==0){
			this.controlador.iniciaReparacion();
		}
		if(caso==1){
			this.controlador.iniciaReparacion();
		}
		if(caso==2){
			this.showError("Configuracion incorrecta, favor revisar\nla configuracion, y luego intente nuevamente.");
		}
	}
